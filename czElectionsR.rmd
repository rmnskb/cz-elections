...
# Checks to what field does candidate's profession belongs to (such as finance, law, medicine etc.) with the help of keywords
...
```
The missing age stands for missing candidates on initial elections paper. These entities can be deleted.
```{r age NA}
snem_long %>%
        filter(is.na(age))
```
As can be seen below, the right-wing ideology prevails among the candidates, so we can impute the said ideology to missing data without the risk of messing up the ratios.
```{r ideology table}
table(snem_long$ideology)
```
```{r NA imputation}
snem_long <- snem_long %>%
        # deleting the dummy NA rows
        filter(!is.na(age)) %>%
        # imputing the majority group into NA values
        replace_na(list(ideology = as.factor("Pravice")))
...
Males and females does not differ much in terms of distribution, albeit female candidates tend to be younger than their male opponents.
```{r age dist}
...
The third field is businesspeople, university degrees are not prevalent in this group.
```{r fields}
snem_long %>%
...
snemTrain <- snemRose[trainIndex, ]
snemTest <- snemRose[-trainIndex, ]

nrow(snemTrain)
nrow(snemTest)
```

5 way cross validation will be used during training process.
...
summary(resamps)
```
As can be seen from the summary above and the plot below, Random Forest has the best ROC metric on training data, picking at ~0.92.
...
Confusion matrix shows the performance of an algorithm, with right predictions being on the main diagonal and wrong predictions on counter diagonal.
The logistic regression defines the right class most times, although it often classifies negative class as positive, which resembles in lower sensitivity rate.
```{r gml metrics 1}
library(yardstick)

conf_mat(snemGml, mandate, pred) %>%
    autoplot(type = "heatmap") +
        labs(title = "Logistic regression's confusion matrix") +
...
classMetrics(snemGml, truth = mandate, estimate = pred, no)
...
Although this algorithm still struggles with assigning the positive class correctly, sensitivity rate still resembles this characteristic.
```{r randf metrics 1}
conf_mat(snemRandf, mandate, pred) %>%
  autoplot(type = "heatmap") +
  labs(title = "Random forest's confusion matrix") +
  scale_fill_gradient(low="#D6EAF8",high = "#2E86C1")
```
```{r randf metrics 2}
classMetrics(snemRandf, truth = mandate, estimate = pred, no)
```
...
conf_mat(snemSvc, mandate, pred) %>%
  autoplot(type = "heatmap") +
  labs(title = "Support vecto classifier's confusion matrix") +
  scale_fill_gradient(low="#D6EAF8",high = "#2E86C1")
```
```{r svc metrics 2}
classMetrics(snemSvc, truth = mandate, estimate = pred, no)
```

### Conclusion
In this report following results have been found out:

* Candidates' age distribution is almost symmetric
* Sex ratio is 3:1 throughout the years
...
Three models were compared during the modelling process: logistic regression, random forest and support vector classifier. The best one of them was random forest algorithm with ROC AUC around 0.92.
Additionally, logistic regression's coefficients may be compared to see what features contribute the most to candidates' chances of winning (according to this model):
```{r glm summary}
summary(glmModel)
```
Age does not contribute much to overall chance, current politicians and lawyers have better chances in winning. Region and sex do not play a huge role. Right-wing ideology may contribute negatively, whilst centrists have higher chances.